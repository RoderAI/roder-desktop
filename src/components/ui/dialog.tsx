...
export function DialogTrigger({
  ...
  ref,
  ...props
}: DialogTriggerProps & { ref?: React.Ref<HTMLButtonElement> }): React.JSX.Element {
...
} & { ref?: React.Ref<React.ElementRef<typeof DialogPrimitive.Popup>> }): React.JSX.Element {
  return (
    <DialogPrimitive.Portal>
      ...
      <DialogPrimitive.Popup
        ref={ref}
        className={cn(
          "dialog-popup fixed left-1/2 top-1/2 z-50 flex max-h-[calc(100vh-theme(spacing.12))] w-full max-w-lg flex-col gap-4 overflow-y-auto rounded-lg bg-popover p-5 text-base text-popover-foreground shadow-lg ring-1 ring-foreground/10 outline-none",
          className,
        )}
        {...props}
      >
...
export function DialogTitle({
  ...
  return <DialogPrimitive.Title ref={ref} className={cn("font-medium text-foreground", className)} {...props} />;
}
