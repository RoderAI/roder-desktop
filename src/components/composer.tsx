import { ArrowDown, Check, FileText, ImageIcon, Plus, Search, Square, X } from "lucide-react";
...
export function Composer({
  ...
  return (
    ...
    >
      ...
      <div className={cn("rounded-[18px] bg-white shadow-sm ring-1 ring-border transition-colors", dragActive && "ring-2 ring-ring")}>
  ...
  return (
    <div className="flex shrink-0 items-center gap-2 text-foreground">
      ...
      >
        ...
        >
          <Combobox.Value>
            {(model: RoderModel | null) => <span>{modelName(model ?? selected)}</span>}
          </Combobox.Value>
          <DropdownTriggerChevron />
        </Combobox.Trigger>
        <Combobox.Portal>
          <Combobox.Positioner align="end" side="top" sideOffset={8} className="z-50">
            ...
            >
              ...
              <Combobox.Empty>
                <div className="px-3.5 py-4 text-sm text-muted-foreground">No matching models</div>
              </Combobox.Empty>
...
const providerLogos: Record<string, ProviderLogoDefinition> = {
  anthropic: {
    title: "Anthropic",
    paths: [{ d: "M13.827 3.52h3.603L24 20h-3.603l-6.57-16.48zm-7.258 0h3.767L16.906 20h-3.674l-1.343-3.461H5.017l-1.344 3.46H0L6.57 3.522zm4.132 9.959L8.453 7.687 6.205 13.48H10.7z" }],
  },
  cohere: {
    title: "Cohere",
    paths: [
  ...
  perplexity: {
    title: "Perplexity",
    paths: [{ d: "M19.785 0v7.272H22.5V17.62h-2.935V24l-7.037-6.194v6.145h-1.091v-6.152L4.392 24v-6.465H1.5V7.188h2.884V0l7.053 6.494V.19h1.09v6.49L19.786 0zm-7.257 9.044v7.319l5.946 5.234V14.44l-5.946-5.397zm-1.099-.08l-5.946 5.398v7.235l5.946-5.234V8.965zm8.136 7.58h1.844V8.349H13.46l6.105 5.54v2.655zm-8.982-8.28H2.59v8.195h1.8v-2.576l6.192-5.62zM5.475 2.476v4.71h5.115l-5.115-4.71zm13.219 0l-5.115 4.71h5.115v-4.71z" }],
  },
  xai: {
    title: "xAI",
    paths: [{ d: "M6.469 8.776L16.512 23h-4.464L2.005 8.776H6.47zm-.004 7.9l2.233 3.164L6.467 23H2l4.465-6.324zM22 2.582V23h-3.659V7.764L22 2.582zM22 1l-9.952 14.095-2.233-3.163L17.533 1H22z" }],
  },
};
...
function ProviderLogo({ provider }: { provider: string }): React.JSX.Element {
  ...
  return (
    <span className="grid size-5 shrink-0 place-items-center text-foreground" title={logo?.title ?? providerName(provider)} aria-hidden="true">
      {logo ? (
