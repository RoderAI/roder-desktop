      ...
      <header className="flex items-start justify-between gap-6 border-b border-border px-5 py-4">
        <div>
          ...
          <p className="mt-1 max-w-[660px] text-base text-muted-foreground">
            ...
            active agent. This mirrors the Claude Desktop connector pattern: enable a local bridge here, then connect
            the browser extension to the local endpoint.
          </p>
        </div>
      ...
      <div className="divide-y divide-border px-5">
        <SettingsRow label="Remote bridge" description="Starts a local authenticated WebSocket bridge for Chrome.">
          <Button
            ...
            onClick={() =>
              void run(running ? window.roderDesktop.chromeBridgeStop : window.roderDesktop.chromeBridgeStart)
            }
          >
        ...
        <SettingsRow
          ...
          description="Paste this into the Roder Browser Bridge extension endpoint field."
        >
          <CopyField
            value={status.url}
            placeholder="Start the bridge to create a URL"
            copied={copied === "url"}
            onCopy={() => void copy(status.url, "url")}
          />
        </SettingsRow>

        <SettingsRow
          label="Bearer token"
          description="Paste this into the extension token field. It changes each time the bridge starts."
        >
          <CopyField
            value={status.token}
            secret
            placeholder="Start the bridge to create a token"
            copied={copied === "token"}
            onCopy={() => void copy(status.token, "token")}
          />
        </SettingsRow>

        <SettingsRow
          label="Pairing link"
          description="For future deep-link pairing support; currently useful for debugging."
        >
          <CopyField
            value={status.pairingUrl}
            placeholder="Unavailable until the bridge is running"
            copied={copied === "pairing"}
            onCopy={() => void copy(status.pairingUrl, "pairing")}
          />
        </SettingsRow>
      </div>
      ...
      <div className="space-y-4 border-t border-border px-5 py-4">
        <div className="rounded-xl border border-border bg-background/60 p-4">
          ...
          <ol className="mt-3 list-decimal space-y-2 pl-5 text-base text-muted-foreground">
            <li>
              Build and load the extension from <span className="font-mono text-foreground">../roder-chrome/dist</span>.
            </li>
        ...
        <div className="flex flex-wrap gap-2">
          <Button
            variant="outline"
            size="sm"
            onClick={() => void run(window.roderDesktop.chromeBridgeRestart)}
            disabled={busy}
          >
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => void window.roderDesktop.chromeBridgeOpenExtensionOptions()}
          >
