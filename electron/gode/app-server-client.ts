...
export class GodeAppServerClient extends EventEmitter {
  #child: ChildProcessWithoutNullStreams | null = null;
  #buffer = "";
  #nextId = 1;
  #pending = new Map<number | string, PendingRequest>();
  #startPromise: Promise<GodeStatus> | null = null;
  #status: GodeStatus = {
    state: "stopped",
    binary: "unresolved",
  };
  ...
  async start(): Promise<GodeStatus> {
  ...
  async #startProcess(): Promise<GodeStatus> {
  ...
  async restart(): Promise<GodeStatus> {
    await this.stop();
    return this.start();
  }
