...
test("browser tool proxy executes calls through BrowserManager", async () => {
  ...
  const result = await callDesktopBrowserTool(browser as never, "browser_navigate", { arguments: { url: "example.com" } });

  expect(result).toEqual({ text: '{"url":"example.com","visible":true}', data: { url: "example.com", visible: true }, is_error: false });
});

test("browser tool proxy reports invalid input as tool errors", async () => {
  const result = await callDesktopBrowserTool({} as never, "browser_navigate", { arguments: {} });
...
test("browser skill injection avoids duplicates from the app-server", () => {
  ...
  const result = mergeDesktopBrowserSkill({ skills: [existingChrome, { ...browserSkill, description: "old" }], diagnostics: [] });
